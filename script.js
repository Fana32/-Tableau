...
function remplirTableau(){
    ...
    // Début
    const debut =
        (pageActuelle - 1) * elementsParPage;
...
function modifierContact(position){
    ...
    prenom.value =
        tab_contact[position].prenom;

    nom.value =
        tab_contact[position].nom;

    tel.value =
        tab_contact[position].tel;
...
function afficherPagination(){
    ...
    const nombrePages =
        Math.ceil(tab_contact.length / elementsParPage);
    // Bouton précédent
    pagination.innerHTML += `
